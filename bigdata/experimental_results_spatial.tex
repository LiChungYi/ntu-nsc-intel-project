...
& \multicolumn{3}{ c|}{Humidity~(\%)} & \multicolumn{3}{ c|}{Light~(lux)} & \multicolumn{3}{ c| }{Temperature~(\degC)} 
& \multicolumn{3}{ c|}{Humidity~(\%)} & \multicolumn{3}{ c|}{Light~(lux)} & \multicolumn{3}{ c  }{Temperature~(\degC)} \\ \hline
...
Table~\ref{table:spatial} compare STR-MF with TR-MF.
...
\subfigure[ Expected similarity]{
	...
	\includegraphics[width=0.15\textwidth]{expected.eps}
}
\hspace{-0.4cm}
\subfigure[Humidity]{
	\label{fig:humidity_similarity}
	\includegraphics[width=0.15\textwidth]{Hum_similarity.eps}
}
\hspace{-0.4cm}
\subfigure[Light]{
	\label{fig:light_similarity}
	\includegraphics[width=0.15\textwidth]{Light_similarity.eps}
}
\vspace{-0.3cm}
...
Second, if knowledge of spatial correlation does to some extent reflect the true correlations between sensors, incorporating this in scenarios with limited information can be helpful (e.g., the humidity imputation for the consecutive missing pattern in Table \ref{table:spatial} improves with spatial information); otherwise it is useless (e.g., no improvement on the light imputation for the consecutive missing pattern in Table \ref{table:spatial}).
