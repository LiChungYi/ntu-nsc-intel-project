...
Hot-deck imputation methods directly fill in the missing values using either neighbor values or historical self records
such as the \textit{last-seen} method, while the prediction models exploit a function 
%(involving metrics other than just sensor values) 
...
However, the former requires accurate spatial coordinates, and
...
knowledge on the mean and covariance function.  In contrast, in this paper we focus on sensor data imputation that does not rely on domain
knowledge.
