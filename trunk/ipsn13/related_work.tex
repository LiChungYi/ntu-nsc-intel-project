...
Finally, conceptually closer to our solution are the Singular Value
...
In \cite{kondrashov2006spatio}, an improved version of EOF model is proposed. 
The major difference is that it considers the temporal ordering:
The readings from one sensor node is copied $M$ times and form $M$ lag-shifted time series.
The modification does boost the performance of EOF,
but it significantly slows down the performance since the data is enlarged by $M$ times.
