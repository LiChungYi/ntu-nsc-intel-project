...
The learning process of Multivariate TR-MF is very similar to that of TR-MF: the objective function and the optimization process remains the same. Yet, there are some important differences: 
...
or
\begin{center}
$t_{m,n,c}=\sum\limits_{k=1}^{K}x_{m,k} y_{n,k} z_{c,k}$
\end{center}
...
&=\mu_m+\mu_n+\mu_c+\sum\limits_{k=1}^{K}{p_{m,k} q_{n,k} w_{c,k}}
\end{aligned}
\end{equation*}
...
\small
\begin{aligned}
&{\mathbf{p}_m}^\prime={\mathbf{p}_m}-\eta(e*\mathbf{q}_n \mathbf{w}_c + \beta \mathbf{p}_m)
\\&{\mathbf{q}_n}^\prime={\mathbf{q}_n}-\eta(e*\mathbf{p}_m \mathbf{w}_c + \beta \mathbf{q}_n)
\\&{\mathbf{w}_c}^\prime={\mathbf{w}_c}-\eta(e*\mathbf{p}_m \mathbf{q}_n + \beta \mathbf{w}_c)
\\&{\mu_m}^\prime=\mu_m-\eta(e+\beta\mu_m)
\\&{\mu_n}^\prime=\mu_n-\eta(e+\beta\mu_n)
\\&{\mu_c}^\prime=\mu_c-\eta(e+\beta\mu_c)
\end{aligned}
\end{equation*}
where $e=t-\hat{t}$. After a round of updating, we then update the model according to temporal regularization :
\begin{equation*}
\small
\begin{aligned}
&{\mathbf{p}_m}^\prime={\mathbf{p}_m}+\eta\gamma(\mathbf{p}_{m+1}-\mathbf{p}_m+\mathbf{p}_{m-1}-\mathbf{p}_m)
\\&{\mu_m}^\prime=\mu_m+\eta\gamma(\mu_{m+1}-\mu_m+\mu_{m-1}-\mu_m)
\end{aligned}
\end{equation*}
