...
In (S)TR-MF, the matrix $\mathbf{P}$ is much larger than $\mathbf{Q}$, and $\mathbf{P}$ is the main focus in our learning process.

Assume we have two attributes: temperature and humidity.
Then, we get $\mathbf{P}_{tem}$ and $\mathbf{Q}_{tem}$ from temperature matrix $\mathbf{R}_{tem}$, and get $\mathbf{P}_{hum}$ and $\mathbf{Q}_{hum}$ from humidity matrix $\mathbf{R}_{hum}$. 
Now we have two $\mathbf{P}$s, and they should be similar or at least correlated because every $\mathbf{p}_m$ represents the factors of time step $m$.
This motivates us to design Multivariate (S)TR-MF.

In Multivariate (S)TR-MF, the $\mathbf{R}$ is horizontally concatenate of the temperature matrix $\mathbf{R}_{tem}$ and $\mathbf{R}_{hum}$
\begin{equation*} \mathbf{R} = \begin{bmatrix}\mathbf{R}_{tem} & \mathbf{R}_{hum} \end{bmatrix} \end{equation*}
The horizontal concatenation makes the temperature model and humidity model share the common $\mathbf{P}$, so they merge the similar factors and communicate the observed information with each other.

Multivariate (S)TR-MF is very similar to (S)TR-MF: the objective function and the optimization process remains the same. 
Yet, there are two important modifications: 
Firstly, for each row (time step), we need two bias terms: one for temperature readings and the other for humidity readings.
