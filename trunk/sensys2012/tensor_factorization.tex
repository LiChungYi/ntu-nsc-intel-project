...
Tucker decomposition is computationally expensive, and researchers have proposed a more efficient Canonical Decomposition~\cite{carrol1970CD} that factorizes a tensor into a sum of component rank-one tensors.
Formally, the Canonical Decomposition~(CD) is the special case of the Tucker decomposition when $\mathbf{S}$ is super-diagonal.
...
\end{equation*}
where $\mathbf{x}_i$,$\mathbf{y}_i$ and $\mathbf{z}_i$ are the column of matrices $\mathbf{X}$, $\mathbf{Y}$ and $\mathbf{Z}$, which are the factor matrices, $K$ is the factor size. 
...
\begin{algorithm}[h]
  ...
  \begin{algorithmic}[1]
    ...
    \State initial the $\mathbf{p}_m, \mathbf{q}_n, \mathbf{w}_c, \mu_m, \mu_n, \mu_c$ for all $m, n, c$
    \Repeat
      \For    {each observed readings t in D}
      \State Update $\mathbf{p}_m, \mathbf{q}_n, \mathbf{w}_c, \mu_m, \mu_n, \mu_c$ 
     \EndFor
     \State Update $\mathbf{p}_m,  \mu_m $ for all $m$
...
\end{aligned}
\end{equation*}
The Tensor Factorization method is summarized in Procedure \ref{alg::conjugateGradient}, which is not hard to implement since it accesses only one row of $\mathbf{P}$, $\mathbf{Q}$, $\mathbf{W}$ at a time.
