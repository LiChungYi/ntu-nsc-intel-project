...
%\end{equation}
where $\hat{y_{i(t)}}$ and $\hat{y_{i(t-1)}}$ represent the signal value in the current and the previous time stamp, and $\alpha$ is the Pearson correlation coefficient between the sensor $i$ and the nearest sensor $j$.
