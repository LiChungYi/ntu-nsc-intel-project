...
Imputation techniques as applied to sensor data can be divided into three categories by the information utilized:
...
The Data Estimation using Physical Model (DEPM)~\cite{li2008data} method employs the basic laws of Physics and 
...
For example, if two sensors are deployed in the kitchen, 
one next to the stove and the other next to the window.
While two sensors are nearby, there may be a large temperature difference
between the two if the stove is in use and the outside temperature is cold.
...
Among the spatio-temporal methods, some are Singular Value Decomposition (SVD)-based methods and they are closest to our
solution. Here the SVD we mean the conventional decomposition method in linear algebra, which can only be applied on a complete matrix.
In contrast, our approach is a factorization process applied directly on an incomplete matrix.
