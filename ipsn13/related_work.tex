...
\multirow{2}{*}{\bf Spatial}& WARM~\cite{le2005estimating},& DEPM~\cite{li2008data},\\ 
...
{\bf Temporal}&&BGP~\cite{osborne2012real},EOF~\cite{beckers2003eof}\\
 \hline \end{tabular}
}
\vspace{-0.1in}
\end{table}
...
%K-NN\cite{pan2010k}, 
and Multi-Im\cite{yuan2000multiple}).
...
Such methods enjoy the advantage of being able to handle categorical sensor data, but the performance is limited in continuous sensor data due to the association rules's requirement to quantize the data.
...
AKE~\cite{pan2010k}, BGP~\cite{osborne2012real}, and EOF~\cite{beckers2003eof}.
...
%The Imputation Method (ImM)~\cite{Lim:robust} learns to combine two temporal predictors, the last-seen predictor
...
AKE adopts linear regression models to describe the spatial relationship among each two sensor nodes. The prediction of target node is the weighted combination of the regression models from K-nearest nodes. If there is no enough spatial information to be exploited, AKE will use temporal information to help.
%\begin{center}
...
Recently, a random process method, called Bayesian Gaussian Process model, had been proposed~\cite{osborne2012real}. 
BGP assumes the current readings are Gassian distribution given the past data.
The parameter, mean and covariance, are changable from time to time based on the mean and covariance function.
Thus, its performance depends on mean and convariance function from prior knowledge.
Conversely, we focus on more general propose system to impute sensor reading without domain knowledges. By the reason, we do not compare with BGP in our experiment.
...
Conventional SVD has a significant limitation as it can only be applied to a complete matrix.
...
singular vectors in SVD) is used to reconstruct the matrix and update the estimation of missing values, 
