...
{\bf Temporal}&&EOF~\cite{beckers2003eof},BGP~\cite{Osborne2011}\\%{}&&&BGP~\cite{Osborne2011} \
 \hline \end{tabular}
}
\vspace{-0.1in}
\end{table}
...
AKE~\cite{pan2010k} and BGP~\cite{Osborne2011}. %and Imputation Method~\cite{Lim:robust}.
%These
...
Recently, a random process method,  called Bayesian Gaussian Process model, had been proposed in 2011~\cite{Osborne2011}. However, its performance depends on a convariance function from prior knowledge. Conversely, we focus on more general propose system to impute sensor reading without domain knowledges. By the reason, we do not compare with BGP in our experiment.
