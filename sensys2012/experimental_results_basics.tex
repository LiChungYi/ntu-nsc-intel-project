\subsection{Results on TR-MF} \label{subsec:exp_basic}%from Chung-Yi
We  linear interpolation~(LI), TR-MF, AKE, DESM and STI on the two data sets. Note that the sensor location information is not available in traffic data set, so we assume the distance between all sensor node pairs are equal for DESM and STI.
