...
Note that we also compare TR-MF with its predecessor the EOF mode. Table 2 shows that TR-MF outperforms EOF significantly in every aspect. We didn't generate more EOF results for other experiments because it requires several round of SVD, which is computational expensive and much less efficiency comparing to other models. 
