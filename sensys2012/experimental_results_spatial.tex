...
Table \ref{table:spatial_random} and \ref{table:spatial_temporal} compares STR-MF with TR-MF.
...
Note that the uppercase STR-MF stands for TR-MF plus strong spatial regularization, while the lowercase sTR-MF represents weaker spatial-regularized TR-MF.
The results show that for the random missing pattern, adding spatial regularization indeed hurts the performance.
We believe this is the case for the random missing cases because the sensor observations alone are adequate for TR-MF to learn the correlations between sensors, hence enforcing similarity between near-by sensors can actually hurt the performance as sensors might not be that correlated.
