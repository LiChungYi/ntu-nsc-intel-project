...
The prediction founction of sensor $i$ is:
%\begin{equation}
$\hat{y_{it}} = (1-\alpha)\hat{y_{i(t-1)}} + (\alpha)\hat{z}$
%\end{equation}

where $\hat{y_{i(t-1)}}$ represents the signal value in the previous time stamp, and $\alpha$ is the Pearson correlation coefficient between the sensor $i$ and the nearest sensor $j$.
The above equation assumes that the $X_i$ and $X_j$ have the similar data fluctuation trend. 
